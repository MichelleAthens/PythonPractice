		...
		if(ajaxRequest.readyState == 4){
			...
			if(response=="yes"){
				...
				for(var i=1;i<len;i++){
					...
					html+='<input type="button" id="button'+examid+'" name="questionlist" id="'+examid+'" onclick="getexam(\''+identification+'\');"';			
					html+='" value="'+examid+'"'+'></td>';
...
function showresults(data){
	  ...
	  console.log(data);
		...
		if(rating = "none"){
			...
			html+="<center><input type='button' onclick='rateup(\""+identification+"\")' value='Rate Up'>";	
			html+="<input type='button' onclick='ratedown(\""+identification+"\")' value='Rate Down'></center>";	
		}
...
function rateup(input){
	var ajaxRequest;
		...
		//ajaxRequest.open("POST", "take_test_rateup.php", true);
		//ajaxRequest.send(JSON.stringify(input));

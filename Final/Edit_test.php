...
<body>
	...
	<div name="editExamTable" id="editExamTable">
		<center>
			...
			<label>Description</label><br><textarea rows="2" cols="50"  name="eDes" id="eDes">Description</textarea><br><br>
			<label>Category
...
var ids=[];
	...
	var texamname = sessionStorage.getItem("examname");
	var texamid = sessionStorage.getItem("examid");
	var tdescription = sessionStorage.getItem("description");
	var tdifficulty = sessionStorage.getItem("difficulty");
	var tcategory = sessionStorage.getItem("category");
	var trateup = sessionStorage.getItem("rateup");
	var tratedown = sessionStorage.getItem("ratedown");
	var tidentification = sessionStorage.getItem("identification");
	
	console.log(texamname);
	document.getElementById("eName").value = texamname;
	document.getElementById("eDes").value = tdescription;
	document.getElementById("qCategory").value = tcategory;
	document.getElementById("qLevel").value = tdifficulty;
		...
		hr.onreadystatechange = function(){
			if(hr.readyState == 4){
				...
				for(var i=0;i<len;i++){
					...
					lefthtml+='<input type="button" id="button'+data[i]['id']+'" name="questionlist" id="'+data[i]['id']+'" onclick="add('+data[i]['id']+');"';			
					...
					righthtml+='<input type="button" onclick="remove('+data[i]['id']+');" name="testlist" id="'+"test"+ data[i]['id'];
...
//=================================================
			...
			hr.onreadystatechange = function(){
				if(hr.readyState == 4){
					...
					for(var i=0;i<qlength;i++){
						var Qquestionid 	= data[i]['questionid'];
						var Qpoints			= data[i]['points'];
						
						Qpoints = parseInt(Qpoints);
						Qquestionid = parseInt(Qquestionid);
						
						add(Qquestionid);
						
						document.getElementById("points"+Qquestionid).value = Qpoints;
						
						console.log(Qquestionid);
			...
			var JSONobject = {"identification":tidentification};

			 
			hr.open("POST","take_test_get.php", true);
			hr.send(JSON.stringify(JSONobject));
...
//	MID_PATH="/Online_part2/middle/";
			...
			return false;
				...
				sendarray.push({oldid:tidentification,username:username,examid:texamid,examname:examname,description:des,category:category,difficulty:level,rateup:trateup,ratedown:tratedown});
				ids.sort();
				...
				if(examname==""){
					...
					ajaxRequest.open("POST", "update_test_submit.php", true); //file get contents, decode, function, CURLS

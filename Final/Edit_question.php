...
<center>
	...
	<div>
		<div>
			...
			<p align="left"><textarea rows = 7 cols=80 name="sample" id="sample" placeholder="Sameple Code (optional)"></textarea><br></p>
...
var  cases = sessionStorage.getItem("cases");
var  level = sessionStorage.getItem("level");
var  category = sessionStorage.getItem("category");
var  author = sessionStorage.getItem("author");
var sample = sessionStorage.getItem("sample");
var rateup = sessionStorage.getItem("rateup");
var ratedown = sessionStorage.getItem("ratedown");

document.getElementById("qDescript").value=question;
document.getElementById("qCategory").value=category;
document.getElementById("qLevel").value=level;
...
var casearray = cases.split("|");
casearray = casearray.filter(v=>v!='');

for(var i = 0; i<casearray.length;i++){
	
	console.log("I is " +i);
	var casearray1 = casearray[i].split(" ");
	
	var identifier = casearray1[0];

	if(identifier =="INPUT"){
		addCase();
		updateCase(casearray[i],j,i);
		
		i++; //Skip the next because its solution cell
...
function updateCase(cases,counter,nextcell){
	var currentcase = arguments[0];
	...
	//console.log(x);
	var casearray1 = currentcase.split(" ");
	...
	if (identifier==="INPUT"){
		var input = casearray1[1];
		console.log("COUNTER IS " +nextcell);
		
		var nextarr = casearray[nextcell+1];
		
		var nextarrarr = nextarr.split(" ");
		var solution = nextarrarr[1];
		...
		var solHTML="";
		solHTML += "<textarea id =sol"+current+" rows=1 cols=30 class='casesol' placeholder='Wanted Solution'>"+solution+"</textarea>";
		solDiv.innerHTML = solHTML;
		
		document.getElementById('text'+current).value=input;
...
//done
				...
				var input = casearray1[1];
				var finding = casearray1[3];			
			
				var wantDiv = document.createElement('div');
				var wantHTML="";
				wantHTML += "<textarea id =want"+current+" rows=1 cols=30 class='wantsol' placeholder='How Many'>"+finding+"</textarea>";
				wantDiv.innerHTML = wantHTML;
				
				document.getElementById('text'+current).value=input;
				document.getElementById(current).selectedIndex = 3;
			...
			if(identifier==="FIND"){
				document.getElementById(current).selectedIndex = 2;
			}
			else if(identifier==="FUNCTION-NAME"){
				document.getElementById(current).selectedIndex = 4;
			}
			...
			newSol += "<br><textarea id='text"+current+"'  rows = 1 cols=30 class = 'casetext'>"+input+"</textarea>";
...
//===================================================
	...
	console.log(sample);
	var myJSONObject={"questionid":questionid,"username":author,"question":description,"sample":sample,"category":category,"level":level,"cases":casearray2,"rateup":rateup,"ratedown":ratedown};
	console.log(myJSONObject);
	
	
		ajaxRequest.open("POST", "Edit_question_back.php", true);
